...
#        print(end - start)
    ...
    obj_flag = torch.empty(output.shape[0],1)
    ...
    # output_w_mi_ == tensor(3,9) 마지막은 obj_id
    ...
    if type(previous_output_with_obj_id) == int: #이전 프레임에서 detect된 obj가 없음
        ...
        for i in range(output_w_mid_coord.shape[0]):
            min_dist_sqr = 999999999
            temp_obj_id = 0
            
            for j in range (previous_output_with_obj_id.shape[0]):
                temp_dist =torch.dot(output_w_mid_coord[i, 1:3], previous_output_with_obj_id[j, 1:3])
                if min_dist_sqr > temp_dist :
                    min_dist_sqr = temp_dist
                    temp_obj_id = previous_output_with_obj_id[j, 8]
                    
            obj_flag[i, 0] = temp_obj_id
    ...
    list(map(lambda x: write_person(x, im_batches, orig_ims), output))
