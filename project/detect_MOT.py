...
#        print(end - start)
    ...
    def write(x, batches, results, obj_id):
        c1 = tuple(x[1:3].int())
        c2 = tuple(x[3:5].int())
        img = results[int(x[0])]
        cls = int(x[7])
    ...
    ##
    list(map(lambda x: write(x, im_batches, orig_ims, x[8]), output))
     
    print(orig_ims)
    
    orig_ims = np.array(orig_ims)
    
    #orig_ims = np.ndarray(orig_ims)
     
    det_names = pd.Series(imlist).apply(lambda x: "{}/det_{}".format(args.det,x.split("/")[-1]))
    
    list(map(cv2.imwrite, det_names, orig_ims))
    print(np.array(det_names))
    d = np.array(det_names)
    
    imgimg = cv2.imread( d[0], cv2.IMREAD_UNCHANGED)
    imgimg = cv2.resize(imgimg, (1200,450+225), cv2.INTER_CUBIC)
    cv2.imshow("result", imgimg )
    cv2.waitKey(0)
