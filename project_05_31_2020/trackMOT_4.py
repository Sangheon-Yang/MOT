...
def arg_parse():
    ...
    parser.add_argument("--display", dest="display", help="Scales to use for detection",
                        default="0", type=str)
...
def track_result(frame_no):
    ...
    while line_count < line.__len__():
        ...
        output_line += str(obj_flag[line_count][0].numpy()) + "\n"
...
if __name__ == '__main__':
    ...
    #object_data_base = torch.randn(1, 18, dtype=torch.int)
            ...
            for i in range(output_w_mid_coord.shape[0]):  # check current frame's detection
                ...
                for j in range(previous_output_with_obj_id.shape[0]):  # check previous frame's detection
                    if output_w_mid_coord[i, 7] == previous_output_with_obj_id[j, 7]:  # two objectd have same class
                        ...
                        temp_dist = (output_w_mid_coord[i, 1] - previous_output_with_obj_id[j, 1]) ** 2 + (
                                    output_w_mid_coord[i, 2] - previous_output_with_obj_id[j, 2]) ** 2

                        # if the distance is minimum and short enough, and the object is not matched yet,
                        if min_dist_sqr > temp_dist and temp_dist < (orig_inp_dim_h * 0.1) ** 2 and float(
                                previous_output_with_obj_id[j, 6]) > 0:
    ...
    for img_id in range(count_limit):
        ...
        if display != 0:
            # draw bound box in original image
            list(map(lambda x: write_(x, curr_img, classes), output))
